...
export class TodoItem extends React.Component<TodoItem.Props, TodoItem.State> {
  ...
  handleSave(id: number, text: string, amount:string) {
    if (text.length === 0) {
      this.props.deleteTodo(id);
    } else {
      this.props.editTodo({ id, text, amount });
    }
    ...
    if (this.state.editing) {
      element = (
        <TodoTextInput
          ...
          onSave={(text, amount) => todo.id && this.handleSave(todo.id, text, amount)}
        />
      ...
      element = (
        ...
        <div className={style.view}>
          ...
          />
          <label className={style.name} onDoubleClick={() => this.handleDoubleClick()}>{todo.text}</label>
          <label className={style.amount} onDoubleClick={() => this.handleDoubleClick()}>{todo.amount}</label>
          <button
